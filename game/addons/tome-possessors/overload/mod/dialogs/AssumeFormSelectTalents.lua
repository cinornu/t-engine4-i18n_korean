...
function _M:init(assumeform, actor, body, on_end)
	...
	Dialog.init(self, ("Assume Form: Select Talents (max talent level %0.1f)"):tformat(self.max_talent_level) , 680, 500)

	self:generateList()

	self.c_ok = Button.new{text=_t"Possess Body", fct=function() self:ok() end}
	self.c_cancel = Button.new{text=_t"Cancel", fct=function() self:cancel() end}
	...
	local help = Textzone.new{width=math.floor(self.iw - self.c_list.w - 20), height=self.ih, no_color_bleed=true, auto_height=true, text=_t"#SLATE##{italic}#Your level of #LIGHT_BLUE#Full Control talent#LAST# is not high enough to use all the talents of this body. Select which to keep, your choice will be permanent for this body and its clones."}
