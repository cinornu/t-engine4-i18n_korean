...
newAI("world_patrol", function(self)
	if not self.energy.used and self.x and self.y then
		if self:runAI("target_world") and self:reactionToward(self.ai_target.actor) < 0 and game.level.map:isBound(self.ai_target.actor.x, self.ai_target.actor.y) and (not self.ai_state.no_follow_beyond or core.fov.distance(self.x, self.y, self.ai_target.actor.x, self.ai_target.actor.y) < self.ai_state.no_follow_beyond) then
