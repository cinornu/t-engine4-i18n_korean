...
-- "Greater" vaults generally contain higher level npc's and loot and can usually appear in most zones.
...
return function(gen, id, lev, old_lev)
	...
	local list = table.clone(gen.data.greater_vaults_list or list)
	if gen.data.greater_vaults_list_add then table.append(list, gen.data.greater_vaults_list_add) end
	...
	repeat
		...
		data.map = vault_exists("vaults/auto/greater/"..vaultid) or vault_exists("vaults/"..vaultid) or vault_exists("vaults/auto/lesser/"..vaultid)
	...
	if not vault then print("greater_vault could not generate a vault") return nil, "greater_vault: no appropriate vaults found" end
	local w = vault_map.w
	local h = vault_map.h
	-- Convert it to a room, copying certain generation data
	return { name="greater_vault-"..vaultid.."-"..w.."x"..h, w=w, h=h,
		...
		generator = function(self, x, y, is_lit)
			...
			for i = x, x + w - 1 do for j = y, y + h - 1 do
				...
				if not gen.map.map[i + j*gen.map.w][Map.TERRAIN] then -- failsafe for undefined grids
					print("[greater_vault] WARNING: replacing undefined tile for", vault.name, i, j)
