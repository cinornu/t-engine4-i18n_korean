...
그렇긴 하지만, <?=player:he_she()?>가 탈출한 이후로 <?=player:his_her()?> 힘이 급격히 강력해졌다는 걸 고려하면, 적절한 전술을 염두에 두는 것이 중요하다. 우리는 우리 종족의 몇몇이 <?=player:he_she()?>의 편에서 싸우는 것이 목격되었다는 충격적인 보고를 들었다. we can assume that <?=player:he_she()?>가 우리 군대의 일부를 노예화하기 위해 뒤틀린 형태의 판의 힘 쓰고있는 것으로 추정할 수 있다. <?=player:his_her()?> 전투 잠재력 중 대부분은 <?=player:his_her()?> 능력에서 기인하기보단 이러한 노예들로부터 나온다. 따라서, 세 가지를 명심하라:

-<?=player:his_her()?> 노예보다 에이알 인을 우선으로 하라. 우리는 아직 <?=player:he_she()?>가 어떻게 우리 동료들을 조종하는지에 대해 구체적으로 알고있는 것이 없다. 허나, 그가 무력화되자마자 정신 지배가 풀릴 것으로 보인다.만약 <?=player:he_she()?>가 울흐'록의 대전사나 그에 버금가는 요원을 장악했다면, 우리는 <?=player:he_she()?>에게 화력을 집중시킬 필요가 있다. 그렇긴하지만, <?=player:he_she()?>를 무효화하는 것이 그러한 생물을 만드는 노력이나 시간을 절약하는 것보다 더 중요하다. 특히, 십자 포화 속에서 매혹된 임프를 잡는 걱세 대해 걱정하지 마라. 그가 무엇을 사용하든 우리는 더 많은 것을 만들 수 있다.
