...
At this point tears began to stream down the ogres face. Continuing to speak between sobs the ogre continued, "I'm not sure why, but a small band attack us." At this point the ogre began to cry to the point that it was unable to speak, and seeing this another ogre continued, "We were bound in chains and taken to a small settlement just beyond this path. There we were subjected to insane experiments, something about 'cleansing us', and we witnessed a great many ogres die in grotesque and horrible ways. Realizing we would meet a similar fate we broke out of our bonds and made to escape from our cages." The ogre paused for a bit, looking behind me at the devastated landscape, then stated, "Although I'm not exactly sure where we may run to."]], [[[i]에이알 전역에서 남해안 만큼 큰 피해를 입은 곳도 없었다. 대륙의 동쪽으로 여행한 적은 없지만, 내가 그곳에서 본 참상만큼 더한 것을 상상할 수가 없다. 이 땅은 절대 원래대로 회복할 수 없을 것이지만, 샬로레에겐 자신들의 잘못을 상기하는 표식이 되리라 믿는다. 다시는 마법을 그런 식으로 오만하게 휘두르지 않도록.[/i]

하플링에게서 들었던 대로 나는 남쪽으로 향했다. 왜 내가 그냥 북쪽의 고향으로 향하지 않았는지는 모르겠지만, 내 안에 끓어오르는 분노 때문에 가면 쓴 자들의 지도자를 만나야겠다고 생각했다. 남족으로 가면 갈수록 하늘은 하늘색에서 보라색으로, 보라색에서 검붉은 색으로 변해갔으며, 이내 태양조차 보이지 않았다. 이 주변의 땅도 마법폭발의 파괴적인 힘으로부터 벗어나지 못 한 게 틀림없었다. 한나절동안 걸은 두에야 나는 불타버린 나무들 옆자리에 천막을 쳤다. 이 파괴된 황야에서 채집할 수 있는 것은 거의 없었으나, 산딸기나 버섯 따위를 조금 찾아내 먹을 수 있었고, 내 식량은 좀 더 아낄 수 있었다.

다음 날 난 여정을 계속했다. 몇 시간 뒤, 남쪽 해안가에 도착했고 거기에서 끔찍한 광경을 목격했다. 바닷물이 마치 물 끓이듯 거품이 올라왔으며, 땅은 새까맣게 타버렸다. 아직 자리를 지키고 있던 초목들은 완전히 타버려 생기를 모두 잃어버렸다. 마법폭발로 파괴된 자연의 잔재는 벌레나 곤충들에게 파먹히지도 않고 그대로 남아있었다. 위를 올려다보자 검은 구름과 시커먼 하늘 뿐이었다. 동쪽에는 땅속에서 용암이 나와 웅덩이를 만든 것이 보였다. 그 용암은 작은 호수를 만들어 붉고 뜨거운 죽음의 흐름을 형성했고, 나는 이것들을 피해 움직이느라 매우 조심해야 했다. 북쪽을 쳐다보자 대지가 불쑥불쑥 튀어나와 하늘로 치솟는 이상한 광경까지 볼 수 있었다.

솟아난 지형으로 가까이 다가가자, 그것이 먼 산봉우리처럼 솟아오르고 있다는 것을 알았다. 언덕을 오르면서, 그 너머로 바위들이 부자연스럽게 협곡과 뾰족한 가시를 형성하고 있는 것이 보였으며, 그 사이를 넘어가려는 건 자살 행위나 다름 없었다. 엘발라에서 후퇴해 온 병사들을 기억하면서, 그들이 고향으로 돌아오기 위해 왜 북쪽을 경유해야 했는지 다시금 깨달았다. 미드베일 평원이 있던 대륙의 중앙은 이제 더 이상 없었다. 평원은 이제 바위가 솟아난 끔찍한 아수라장이 되어 들어오는 자들을 모두 죽여버릴 곳이고, 마법폭발 이전에 그곳에서 살았던 자들이 맞이한 운명도 그랬을 것이다.

나는 조심스럽게 처참한 모습의 대지를 따라 여정을 이어갔다. 동쪽으로 가는 동안 태양빛은 피어오르는 연기에 파묻혀 거의 보이지 않았고, 나는 가는 길에 튀어나오는 용암에서 나오는 빛에 의지하여 발을 디뎌야 했다. 며칠이나 그곳에서 걸었는지 제대로 기억나지 않는다. 어쨌든 그 땅에서 벗어나는 건 굉장히 오래 걸리는 일이었다. 마침내 나는 용암의 강이 바다까지 쭉 이어져 뻗어나가는 광경을 보게 되었다. 그 너머로 한때 숲이었던 곳이 불탄 것이 보였다. 그곳을 지나갈 수는 없었기에, 나는 주변을 둘러보다 북쪽을 보았고, 앞길을 가로막는 장애물이 없어보였다. 그래서 나는 그쪽으로 방향을 돌려 안전한 길이 있는지 확인하고자 했다.

나는 높은 언덕으로 올라가 주변을 살폈고, 솟아오른 산맥 사이로 조그마한 협곡을 발견했다. 내가 지금까지 지나온 땅은 구릉이 많고 험했으나, 위험하기 짝이 없는 땅의 균열과 낭떠러지들 사이에서도 나는 어떻게든 발 디딜 곳을 찾아 넘어올 수 있었다. 천천히 언덕에서 내려가며, 나는 저 산등성이의 반대편으로 넘어갈 수 있는 길을 찾았다. 그런데 갑자기 저 멀리에서 내쪽으로 빠르게 다가오는 어떤 무리를 발견했다. 그 무리 중 하나가 나를 발견하고 다른 이들에게 소리치고선 속도를 늦춰 다가왔다. 저들이 다가오는 사이, 나는 저들이 대체 무슨 종족인지 잠깐 생각에 빠졌다.

그들은 키가 컸다. 나보다도 컸다. 게다가 꽤 멀리서도 우락부락한 근육이 보였다. 그들의 피부에서 무언가 빛나고 있었는데, 그들이 더 가까이 다가오자 나는 그것이 엘발라에서 본 룬의 각인과 비슷하다는 것을 깨달았다. 목소리가 들릴 정도의 거리에서 그들 중 하나가 소리쳤다. "우리들을 잡아갈 생각이라면, 한바탕 싸울 준비나 해라!" 이때 그들 또한 가면을 쓴 미친 자들의 희생양이라는 것을 깨달았다. 난 적대할 생각이 없다는 것을 보여주기 위해 짐을 땅바닥에 내려놓고 뒤로 몇 걸음 물러섰다. 이런 행동을 본 뒤 그들은 조금 누그러진 듯했으나, 여전히 신경이 곤두선 채였다.
