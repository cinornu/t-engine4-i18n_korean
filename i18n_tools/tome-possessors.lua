...
t("Possessors are a rare breed of psionics. Some call them body snatchers. Some call them nightmarish.", "빙의술사는 매우 희귀한 초능력자입니다. 그들은 육체 강탈자 혹은 악몽 같은 존재라고 불리기도 합니다.", "_t")
...
t([[Your mind is so powerful it can bend reality, providing you with an extra-natural #{italic}#storage#{normal}# for bodies you snatch.
		You can store up to %d bodies.]], [[정신의 힘이 너무 강한 나머지, 현실 구조를 뒤틀리게하여, 시전자가 빼앗은 육체를 보관할 초자연적 #{italic}#보관소#{normal}#를 이용할 수 있게 됩니다.
...
t("You require need to assume a form first.", "우선 형태를 취해야 한다.", "logPlayer")
...
t([[When you assume a form you may cannibalize a body in your reserve to replenish your current body.
		...
		동일하거나, 상위 등급의 육체만이 이 효과를 위해 사용될 수 있습니다. 이 기술을 사용할 때마다, 해당 육체에 대한 회복량은 33%% 씩 감소하게 됩니다.
		현재 육체는 부품이 된 육체가 가진 최대 생명력의 %d%% 만큼 회복되며, 해당 수치의 50%% 만큼 염력을 회복합니다.
		회복 효과는 실제 회복 효과보다 자연적이 아닌, 초능력적인 성질을 훨씬 많이 띄고있습니다. 따라서 이로 발생하는 회복은 회복을 막는 여러 기술들을 무시하고 작동합니다.
...
t([[Your mere presence is a blight in your foes minds. Using this link you are able to reach out and steal a talent from a target.
		...
		강탈한 기술은 사용 시, 어떤 원천력도 소모하지 않습니다.
		기술 레벨 5 일 때, 강탈할 기술을 선택할 수 있게 됩니다.
...
t([[For a brief moment your whole body becomes etheral and you dash into a nearby creature and all those in straight line behind it (in range %d).
		...
		]], [[짧은 시간 동안, 몸 전체를 영체로 바꿔 가까운 대상을 향해 질주합니다. 사거리 %d 내의 모든 개체를 무시하고 대상에게 질주합니다.
...
t([[Your physical form is but a mere extension of your mind, you can bend it at will for %d turns.
		...
		지속 시간 동안 모든 저항 %d%% 와 치명타 무시 확률 %d%% 를 얻게 됩니다.
		이 기술을 발동할 때, 최대 %d 개의 정신적 또는 물리적 효과를 제거합니다.
		]], "tformat")
t("Ominous Form", "불길한 형체", "talent name")
t("You are already assuming a form.", "당신은 이미 형태를 취하고 있다.", "logPlayer")
...
t([[Your psionic powers have no limits. You are now able to assault a target and clone its body without killing it.
		...
		]], [[시전자의 초능력은 한계를 모릅니다. 적에게 맹공을 가해, 적을 쓰러뜨리지 않고 적을 복제할 수 있게 됩니다.
		이 형체는 일시적인 것으로, 오직 %d 턴 동안 지속되며 시전자의 정상적인 능력과 같은 수준의 제한을 받습니다.
		이렇게 형태를 취할 때, 시전자의 생명력은 대상에게 속박됩니다. (시전자의 생명력 비율은 언제나 대상의 생명력 비율과 같게 됩니다)
		]], "tformat")
...
t([[You call upon one of your reserve bodies, assuming its form.
		...
		다른 개체의 형태를 취할 때, 현재 장착한 장비들은 시전자에게로 #{italic}#융합#{normal}# 되어, 장비를 벗거나, 새로 장착할 수 없습니다.
		빙의한 육체:
		%s]], "tformat")
...
t([[You cast a psionic web at a target that lasts for %d turns. Each turn it deals %0.2f mind damage.
		...
		초능력 망의 영향을 받고 있는 도중, 대상이 죽으면, 시전자가 해당 육체를 강탈해, 숨겨진 초능력 보관소에 안치합니다.
...
t([[When you assume the form of another body you gain %d%% of the values (defences, crits, powers, save, ...) of the body.
		In addition talents gained from bodies are limited to level %0.1f.]], [[육체의 형태를 취할 때, 해당 육체의 능력치가 %d%% 향상됩니다. (회피도, 치명타, 내성 등등 ...)
		추가로, 육체의 형태를 취할 때, 해당 육체로 얻게되는 기술의 레벨 상한이 %0.1f 이 됩니다.]], "tformat")
...
t([[You shatter your weapon in the ground, projecting a psionic shockwave in a cone of radius %d.
		...
		If you do not have a two handed weapon equiped, but have it in your off set, you instantly automatically switch.]], [[무기를 지면에 던저, 무기를 산산조각 내고, 반경 %d 칸의 원뿔 범위에 초능력 충격파를 풀어놓습니다.
...
t("#ORANGE#%s channels pain to %s!", "#ORANGE#%s 고통을 %s에게 모은다!", "logSeen", nil, {"는",nil})
...
t([[As long as you have at least a stack of Sadist whenever you take damage you use %d psi to harness your stacks of Sadist to divide the damage by your stacks + 1.
		...
		This effect can only happen once per turn and only triggers for hits over 10%% of your max life.]], [[사디스트 중첩을 하나 이상 갖고 있을 때, 피격 시 염력을 %d 소모해, 피해량을 사디스트 중첩 + 1 의 수치로 나눕니다.
		이 효과가 발생할 때마다, 시야 내의 모든, 생명력 80%% 이하의 적이 반동을 받아, 흡수된 피해량의 %d%% 를 대신 받습니다.
		이 효과는 한 턴에 1 번만 일어나며, 한 번에 최대 생명력의 10%% 이상의 피해를 받을 때만 발동됩니다.]], "tformat")
...
t([[As long as you have at least a stack of Sadist you can radiate agony to all those you see in radius %d with 80%% or lower life left.
		For 5 turns their mind will be so focused on their own pain that they will deal %d%% less damage to you.]], [[사디스트 중첩을 하나 이상 갖고 있을 때, 격통을 반경 %d 칸 내의 시전자가 볼 수 있는, 생명력 80%% 이하인 적에게 발산할 수 있게 됩니다.
...
t([[As long as you have at least a stack of Sadist you can mentally lash out at a target, sending horrible images to its mind.
		The target will reel from the effect for %d turns, rendering %d random talents unusable for the duration.]], [[사디스트 중첩을 하나 이상 갖고 있을 때, 대상에게 끔찍한 환상을 보내 정신적인 공격을 가합니다.
		이 효과로 인해 대상은 %d 턴 동안 정신을 놓아, 무작위한 %d 개의 기술을 사용할 수 없게 됩니다.]], "tformat")
...
t("was killed by possession aftershock", "빙의 충격으로 인해 사망했다.", "_t")
...
t("The target is reeling from the aftershock of a destroyed possessed body, reducing damage by 60%%, reducing movement speed by 50%%.", "대상의 빙의체가 파괴되어 그 충격으로 인해 휘청이고 있다. 피해량이 60%% 감소하고, 이동 속도가 50%% 감소했다.", "tformat")
...
t("All resists increased by %d%%, chance to be crit reduced by %d%%.", "모든 저항이 %d%% 증가하고, 받는 치명타 확률이 %d%% 감소합니다.", "tformat")
...
t("You stole your current form and share damage and healing with it.", "피해와 회복을 강탈한 현재 형태와 나눕니다.", "_t")
t("You use the body of one of your fallen victims. You can not heal in this form.", "쓰러진 희생양들 중 하나의 육체를 사용하고 있습니다. 이 형태로는 회복이 불가합니다.", "_t")
t("#CRIMSON#While you assume a form you may not levelup. All exp gains are delayed and will be granted when you reintegrate your own body.", "#CRIMSON#형태를 취하고 있는 동안 레벨 상승이 불가능합니다. 모든 경험치 습득이 지연되지만, 원래 모습으로 돌아올 때 지연된 경험치를 한꺼번에 획득합니다.", "_t")
t("#CRIMSON#Your body died! You quickly return to your normal one but the shock is terrible!", "#CRIMSON#육체가 죽었습니다!  원래 육체로 돌아왔지만, 그 충격은 실로 끔찍합니다!", "say")
t("Your possessed body's eyelids briefly flutter, and a tear rolls down its cheek. You didn't tell it to do that.", "빙의된 육체의 눈꺼풀이 짧은 시간동안 흔들리고, 눈물이 볼을 타고 흐릅니다. 당신은 영문을 알 수 없습니다.", "_t")
t("The flames surrounding Shasshhiy'Kaish slowly die as she falls to her knees.  \"Fiend...  and I thought #{italic}#I#{normal}# could cause suffering.  It's the one thing Eyalites always did best,\" she spits.  \"I heard what had happened to him, and my followers have given more than enough of their life to restore me after this.  All you've accomplished here - [cough] - is giving us a worthwhile new goal...  and target.  All will be repaid tenfold, Eyalite.\"  Her coughing grows weaker, until she abruptly bursts into flame; her ashes scatter into the wind.", "샤쉬'카이쉬가 마침내 무릎 꿇자, 주위를 휘감던 화염이 느리게 꺼저갔습니다.  \"악귀같은 놈...  나는 #{italic}#오직#{normal}# 나만이 고통을 퍼뜨릴 수 있다고 생각했었지.  하지만 그건 네가 언제나 행하는 일인 것 같구나, 에이알인.\" 그녀가 내뱉었다.  \"그에게 어떤 일이 일어났는지 들었다. 그리고 그 사건이 있은 후, 내 추종자들은 나를 회복시키기 위해 그들 목숨을 희생했지.  네가 여기서 행한 모든 것은 - [기침] - 우리에게 가치있는 새 목표를...  각인시키는구나.  네가 행한 모든 것은 열 배로 갚게될 것이다, 에이알인.\"  그녀의 기침은 점점 약해지더니, 갑작스러운 화염 폭발에 그녀의 육체가 휘말립니다. 이윽고, 그녀의 재는 바람에 의해 산산히 흩어집니다.", "_t")
t("Aeryn's bewildered and terrified cries grow quiet, but...  your ears don't ring or hurt as screams of horror and rage surround you, louder than should be deafening.  When they shift to accusations, an unfamiliar guilt dominates your thoughts; you are forced to abandon your body before it can compel you to punish yourself.", "아에린의 당혹스럽고 겁에질린 절규는 이윽고 조용해집니다. 하지만...  분노와 공포에 질린, 소음보다는 굉음에 가까운 비명소리가 당신을 둘러싸고 있음에도 당신의 귀는 울리지도, 아프지도 않습니다.  적들이 비난하자, 익숙하지 않은 죄책감이 정신을 지배합니다. 죄책감이 당신 스스로를 압도하기 전에 이 육체를 포기해야 합니다.", "_t")
...
t("#SLATE##{italic}#Choose which body to assume. Bodies can never be healed and once they reach 0 life they are permanently destroyed.", "#SLATE##{italic}#형태를 취할 육체를 고르십시오. 선택한 육체는 회복이 불가능하고, 생명력이 0 에 도달하면 영구적으로 파괴됩니다.", "_t")
...
t("You have no bodies to use.", "사용할 수 있는 육체가 없습니다.", "logPlayer")
t("Discard Body", "육체 폐기", "_t")
t("Destroy: %s", "파괴: %s", "tformat")
t("Destroy the most damage copy or all?", "가장 많은 피해를 받은 육체를 파괴하겠습니까 혹은 육체 전부를 파괴하겠습니까?", "_t")
t("Most damaged", "가장 많은 피해", "_t")
t("Destroy it?", "파괴합니까?", "_t")
t("#AQUAMARINE#You cannot destroy a body you are currently possessing.", "#AQUAMARINE#현재 빙의 중인 육체를 파괴할 수는 없습니다.", "log")
t("#AQUAMARINE#You are already using that body!", "#AQUAMARINE#당신은 이미 그 육체를 사용하고 있습니다!", "log")
...
t("Assume Form: Select Talents (max talent level %0.1f)", "형태 취하기: 기술을 고르십시오 (최대 기술 레벨 %0.1f)", "tformat")
t("Possess Body", "육체 빙의", "_t")
